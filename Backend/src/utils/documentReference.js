...
const parseLegacyReference = (value = '') => {
  ...
  if (modernMatch) {
    return { branchSegment: modernMatch[1], year: modernMatch[2], serial: modernMatch[3], alreadyModern: true };
  }

  const dmsMatch = /^DMS\/([^/]+)\/(\d{4})\/([^/]+)$/i.exec(normalized);
  if (dmsMatch) {
    return { branchSegment: dmsMatch[1], year: dmsMatch[2], serial: dmsMatch[3], alreadyModern: false };
  }

  const docWithTenantMatch = /^DOC\/[^/]+\/([^/]+)\/(\d{4})\/([^/]+)$/i.exec(normalized);
  if (docWithTenantMatch) {
    return { branchSegment: docWithTenantMatch[1], year: docWithTenantMatch[2], serial: docWithTenantMatch[3], alreadyModern: false };
  }

  const docWithoutTenantMatch = /^DOC\/([^/]+)\/(\d{4})\/([^/]+)$/i.exec(normalized);
  if (docWithoutTenantMatch) {
    return { branchSegment: docWithoutTenantMatch[1], year: docWithoutTenantMatch[2], serial: docWithoutTenantMatch[3], alreadyModern: false };
  }
